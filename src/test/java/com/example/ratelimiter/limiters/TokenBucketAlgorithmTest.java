...
class TokenBucketAlgorithmTest {
    ...
    private final RateLimitPolicy policy = new RateLimitPolicy(10, Duration.ofSeconds(10), AlgorithmType.TOKEN_BUCKET,
            ClientType.IP);
    ...
    @Test
    @DisplayName("Over limit → retryAfter calculated based on current tokens")
    void rejected() {
        ...
        // Need to wait until 1.0 tokens are available. Current is 0.2. Remaining needed
        // = 0.8.
        // With refill rate of 1.0/sec, wait time is ceil(0.8 / 1.0) = 1s.
        assertThat(r.retryAfterSeconds()).isEqualTo(1L);
        // Reset to full capacity (10 tokens) from 0.2: (10 - 0.2) / 1.0 = 9.8. ceil =
        // 10s.
