...
class RateLimiterResolverTest {
    ...
    @Mock
    private RateLimiter auth, upload, sensitive, general;
    @Mock
    private HttpServletRequest request;

    @BeforeEach
    void setUp() {
        resolver = new RateLimiterResolver(auth, upload, sensitive, general);
    }

    @Test
    @DisplayName("/api/auth/login → auth")
    void authRoute() {
    ...
    @Test
    @DisplayName("/api/payment → sensitive")
    void paymentRoute() {
    ...
    @Test
    @DisplayName("DELETE /api/users/5 → sensitive")
    ...
    @Test
    @DisplayName("GET /api/users/5 → general (not DELETE)")
    void getUserRoute() {
        when(request.getRequestURI()).thenReturn("/api/users/5");
